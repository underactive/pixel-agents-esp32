...
| S1 | ES8311 audio codec + speaker amp (CYD-S3) | Sound effects playback |
...
| 6 | I2S_DIN | I2S data in (mic, unused) |
...
#### 10. Audio / Sound System (`sound.cpp`, CYD-S3)
- I2S output to ES8311 codec (I2C control), AP_ENABLE gate pin, and MCLK/BCLK/WS/DOUT pinout
...
- Amp enabled per-clip (in `startClip()`), disabled when playback ends (in `update()`)
...
| `HAS_SOUND` | Enables startup sound playback (auto-defined in `config.h` for `BOARD_CYD_S3`) | Auto |
...
- `firmware/src/config.h` -- derived defines (`HAS_LED`, `LED_TYPE_PWM`, `LED_TYPE_NEOPIXEL`) set from board selection
