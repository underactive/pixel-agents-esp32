...
[Claude Code CLI] --> writes JSONL --> [Python companion] --> USB Serial --> [ESP32 + TFT]
                  ...
                  --> writes JSONL --> [macOS companion app] --> USB Serial / BLE --> [ESP32 + TFT]
                                              |
...
- `firmware/src/led_ambient.h/.cpp` -- RGB LED ambient indicator (CYD only, `#if defined(BOARD_CYD)`)
...
#### 9. LED Ambient (`led_ambient.cpp`, CYD only)
- Active-low common-anode RGB LED on pins R=4, G=16, B=17 (PWM 5kHz, 8-bit)
...
#### 10. macOS Companion App (`macos/PixelAgents/`)
...
- FSEvents-based transcript watching (same `~/.claude/projects/` directory)
...
|----------|---------|--------|
| `BOARD_CYD` | Build-time board selection, enables LED ambient | Defined in CYD env only |
| `HAS_TOUCH` | Enables touch input + hamburger menu compilation | Defined in CYD env only |
...
| `firmware/src/led_ambient.h/.cpp` | RGB LED ambient indicator (CYD only) |
